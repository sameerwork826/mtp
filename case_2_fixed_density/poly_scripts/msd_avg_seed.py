...
def extract_system_size(filename):
    """Extract system size from filename (e.g., msd_28_10_seed1 (1).dat -> 28)"""
...
def extract_seed(filename):
    """Extract seed number from filename (e.g., msd_28_10_seed1 (1).dat -> 1)"""
...
def load_and_average_msd(directory, system_size):
    ...
    pattern = f"msd_{system_size}_*seed*.dat"
...
def plot_msd_normal(directory, system_sizes, plot_title="Mean Square Displacement"):
    ...
    colors = ['blue', 'red', 'green', 'purple', 'orange', 'teal']  # Colors for different system sizes
    markers = ['o', 's', '^', 'D', 'v', 'x']  # Different markers for different system sizes

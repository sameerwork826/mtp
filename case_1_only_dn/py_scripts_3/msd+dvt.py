...
def plot_diffusion_coefficient(directory, system_sizes, plot_title="Diffusion Coefficient"):
    """Plot Diffusion Coefficient (MSD/6t) for multiple system sizes"""
    plt.figure(figsize=(10, 8))
    ...
    for i, size in enumerate(system_sizes):
        time, avg_msd, std_msd = load_and_average_msd(directory, size)
        if time is not None:
            # Calculate diffusion coefficient (avoiding divide by zero)
            # Use np.divide with a small safety offset to prevent divide by zero warnings
            diffusion_coeff = np.divide(avg_msd, 6 * time, out=np.zeros_like(avg_msd), where=time!=0)
            
            # Calculate standard deviation of diffusion coefficient
            # Propagate error using error propagation formula for division
            # Assuming independent errors, though this is a rough approximation
            diffusion_std = np.divide(std_msd, 6 * time, out=np.zeros_like(std_msd), where=time!=0)
            
            color = colors[i % len(colors)]
            marker = markers[i % len(markers)]
            
            # Skip the first point (time=0 can cause issues)
            plt.plot(time[1:], diffusion_coeff[1:], marker=marker, markersize=4, 
                     label=f"System Size {size}", color=color, linestyle='-', 
                     markevery=max(1, len(time[1:])//20))
            
            # Add error bars
            plt.fill_between(time[1:], 
                             diffusion_coeff[1:] - diffusion_std[1:], 
                             diffusion_coeff[1:] + diffusion_std[1:], 
                             alpha=0.2, color=color)
    
    plt.xlabel("Time", fontsize=14)
    plt.ylabel("Diffusion Coefficient (D = MSD/6t)", fontsize=14)
    plt.title(f"{plot_title}", fontsize=16)
    plt.grid(True)
    plt.legend(fontsize=12)
    plt.tight_layout()
    plt.savefig("diffusion_coefficient.png", dpi=300)
    plt.show()
...
if __name__ == "__main__":
    ...
    # Create diffusion coefficient plot
    plot_diffusion_coefficient(data_directory, system_sizes)